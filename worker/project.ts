...
export const projectAgentSchema = z.object({
  ...
  plugins: z.array(z.object({
    ...
    format: z.enum(['stingy', 'codex-agent-plugin', 'dsh-bundle', 'mcp', 'agent-skill']),
    description: z.string().max(2_000),
    supportedFeatures: z.array(z.string().max(200)).max(12),
  })).max(20).default([]),
});
...
export async function runProjectAgent(env: WorkerEnv, input: z.infer<typeof projectAgentSchema>, personalAssistant?: PersonalAssistantCredentials | string, requestId?: string) {
  ...
  const system = [
    ...
    'plugins 仅包含已安装并启用的能力元数据。可以据此采用相应工作流，但不得声称执行了未提供工具结果的 MCP、命令或 Hook。',
  ].join('\n');
  const context = JSON.stringify({ task: input.prompt, permissionMode: input.permissionMode, activeFile: input.activeFile, fileIndex: input.fileIndex, plugins: input.plugins });
